...
import happybase,urllib,urllib2
from lxml import etree
from langconv import *
TableName='TWUser'
ColumnName='profile:description'
ColumnName2='profile:location'
language=ColumnName.split(':')[0]+':languge'
TranslateColumnName=ColumnName+'Translation'
TranslateColumnName2=ColumnName2+'Translation'
...
def TranslationinMySQL(TableName,ColumnName,ColumnName2,language,TranslateColumnName,TranslateColumnName2):
    connection = happybase.Connection('192.168.5.171')
    print connection.tables()
    table = connection.table(TableName)
    for key, data in table.scan():  #查看表中行row
        if (not TranslateColumnName in data.keys()):
             text=data[ColumnName]
             text2=data[ColumnName2]
             translateResult=translate(text)
             translateResult2=translate(text2)
             if translateResult[0]=="NULL":#获取翻译语言
                    LanguageResult=translateResult2[0]
             else:
                    LanguageResult=translateResult[0]
             table.put(key, {language:LanguageResult,TranslateColumnName: translateResult[1],TranslateColumnName2: translateResult2[1]})
             print('RowKey=%s have been translated!'%key)
             # row = table.row(key) #查看翻译结果
             # print row[language],row[ColumnName],row[TranslateColumnName],row[ColumnName2],row[TranslateColumnName2],
    print 'Translation Complete!'

TranslationinMySQL(TableName,ColumnName,ColumnName2,language,TranslateColumnName,TranslateColumnName2)