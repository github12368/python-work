# -*- coding: utf-8 -*-
import time,MySQLdb,sys,urllib,urllib2,re
...
def DeleteProperty(TableName):
    ...
    try:
        ...
        if(conn != None):
            ...
            cur = conn.cursor()
            Command=" alter table "+TableName+" drop language , drop translation"
            cur.execute(Command)
            conn.commit()
            cur.close()
            conn.close()
...
def AddProperty(TableName):
    ...
    try:
        ...
        if(conn != None):
            ...
            print now + "  MySQL Connecting OK!"
            cur = conn.cursor()
            Command=" alter table "+TableName+" add language varchar(30) default null,add translation varchar (1000) default null"
            cur.execute(Command)
...
def TranslationinMySQL(TableName,ColuName):
    # Save the Register user's Information to MySQL
    try:
        conn = MySQLdb.connect(host=mysqldbip,user=mysqldbuser,passwd=mysqldbpsw,db=dbname,port=3306,charset='utf8')
        conn2 = MySQLdb.connect(host=mysqldbip,user=mysqldbuser,passwd=mysqldbpsw,db=dbname,port=3306,charset='utf8')
        if((conn != None)&(conn2!=None)):
            ...
            for i in range(numrows):
                ...
                UpdateCommand=" update "+TableName+" set "+'language="'+translateResult[0]+'" ,translation="'+translateResult[1]+'" where id='+str(row[0])
...
AddPropertyRe=AddProperty(TableName)
if AddPropertyRe==1:
...
# DeleteProperty(TableName) #删除属性
TranslationinMySQL(TableName,ColuName)