# -*- coding: utf-8 -*-
import time,pymysql,urllib,urllib2,requests
from lxml import etree
from langconv import *
reload(sys)
sys.setdefaultencoding('utf-8')
mysqldbip = "192.168.5.121"
mysqldbuser = "root"
mysqldbpsw = "ibm18911029778"
dbname = "showcase2016"
TableName='buaa_user_profileinfo'
ColuName='location'
PostColuname1='location_lat'
PostColuname2='location_lng'
...
def GetLocation(text):
    ...
    url='http://www.google.cn/maps/search/'+text
    hea = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/31.0.1650.63 Safari/537.36'}

    html = requests.get(url, headers=hea)
    html.encoding = 'utf-8'
    resultX=re.findall("center=(.*?)%2C",html.text)#
    resultY = re.findall("%2C(.*?)&amp", html.text)  #
    return resultX[0],resultY[0]

def DeleteProperty(TableName,PostColuname1,PostColuname2):
    ...
    try:
        ...
        if(conn != None):
            ...
            cur = conn.cursor()
            Command=" alter table "+TableName+" drop "+PostColuname1+", drop "+PostColuname2
            cur.execute(Command)
            conn.commit()
            cur.close()
            conn.close()
...
def AddProperty(TableName,PostColuname1,PostColuname2):
    # Save the Register user's Information to MySQL
    try:
        conn = pymysql.connect(host=mysqldbip,user=mysqldbuser,passwd=mysqldbpsw,db=dbname,port=3306,charset='utf8')
        if(conn != None):
            ...
            print now + "  MySQL Connecting OK!"
            cur = conn.cursor()
            Command=" alter table "+TableName+" add "+PostColuname1+" double default null,add "+PostColuname2+" double default null"
            cur.execute(Command)
...
def GetLocationInMySQL(TableName,ColuName,PostColuname1,PostColuname2):
    # Save the Register user's Information to MySQL
    try:
        conn =pymysql.connect(host=mysqldbip,user=mysqldbuser,passwd=mysqldbpsw,db=dbname,port=3306,charset='utf8')
        ...
        if((conn != None)&(conn2!=None)):
            ...
            idNum=ColUNameDict['id']

            START='Trans the "'+ColuName+' in table "'+TableName+'" start!'
            print START

            ReadCommand=" SELECT * FROM "+TableName+" where "+PostColuname1+" is null "#读取未翻译的记录
            cur.execute(ReadCommand)
            conn.commit()
            resultall = cur.fetchall()
            for row in resultall:
                if row[ColuNum]=='' or row[ColuNum]==None:
                    transResult = ['0','0']
                else:
                    try:
                        transResult=GetLocation(row[ColuNum])
                    except:
                        transResult=['0','0']
                print row[idNum],row[ColuNum], transResult[0],transResult[1]
                UpdateCommand=" update "+TableName+" set "+PostColuname1+'="'+transResult[0]+'" ,'+PostColuname2+'="'+transResult[1]+'" where id='+str(row[idNum])
...
# AddPropertyRe=AddProperty(TableName,PostColuname1,PostColuname2)
# if AddPropertyRe==1:
...
#DeleteProperty(TableName,PostColuname1,PostColuname2) #删除属性
GetLocationInMySQL(TableName,ColuName,PostColuname1,PostColuname2)
