...
def TranslationinMySQL(TableName,ColumnName,language,TranslateColumnName):
    ...
    for key, data in table.scan():  #查看表中行row
        if (not TranslateColumnName in data.keys()):
             ...
             table.put(key, {language: translateResult[0],TranslateColumnName: translateResult[1]})
             print('RowKey=%s have been translated!'%key)
             # row = table.row(key) #查看翻译结果
             # print row[language],row[ColumnName],row[TranslateColumnName]
    print 'Translation Complete!'
TranslationinMySQL(TableName,ColumnName,language,TranslateColumnName)